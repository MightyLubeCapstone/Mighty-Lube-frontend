...
class CommonWidgets {
  ...
  static Widget buildDropdownFieldError<T>(String label, List<String> options, int? dropdownSelection, Function(dynamic) onChanged, {String? errorText, bool? isEditable = true}) {
  ...
  // Text Field
  static Widget buildTextField(String hintText, TextEditingController controller, {String? errorText, int linenum = 1, bool isEditable = true, Function(String)? callback}) {
    OutlineInputBorder borderStyle(Color color) {
      return OutlineInputBorder(
        borderRadius: BorderRadius.circular(12),
        borderSide: BorderSide(
...
class TemplateAWidgetState extends State<TemplateAWidget> {
  ...
  void dispose() {
    ...
    _controllers.forEach((key, controller) {
      if (data[key] == null && controller.text != "") {
        data[key] = controller.text;
      }
    });
    // Collect dropdown values
    _dropdowns.forEach((key, value) {
      if (data[key] == null && value != null) {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _buildSwitch("Existing Monitoring System", 'existingMonitor'),
      ...
      _buildSwitch("Portable Monitoring System", 'typeMonitor'),
      if (_toggles['typeMonitor'] == false) ...[
        _buildSwitch("Paint Marker", 'paintMarker'),
      ] else ...[
        _buildSwitch("Drive Motor Amp", 'driveMotorAmp'),
        if (_toggles['driveMotorAmp']!)
          _buildTextField("Drive Motor Amp Quantity", 'driveMotorAmpNum'),
        _buildSwitch("Drive Take-up – Air", 'driveTakeUpAir'),
        if (_toggles['driveTakeUpAir']!)
          _buildTextField("Drive Take-up – Air Quantity", 'driveTakeUpAirNum'),
        _buildSwitch("Take-Up Distance", 'takeUpDistance'),
        if (_toggles['takeUpDistance']!)
          _buildTextField("Take-Up Distance Quantity", 'takeUpDistanceNum'),
        ...
        if (_toggles['driveVibration']!)
          _buildTextField("Drive Motor Vibration Quantity", 'driveVibrationNum'),
    ...
    return Column(
      ...
      children: [
        CommonWidgets.buildSectionTitle(
            "OPCO Free Rail 314 \"Load\" Wheel Greaser - General Information"),
        ...
        _dropdown("Wheel Manufacturer", 'wheelManufacturer', [
          'Green Line',
          'Frost',
          'M&M',
          'Stork',
        ...
        _dropdown(
            "Indexing or Variable Speed Conditions", 'conveyorSpeedUnit', ['Indexing', 'Variable']),
        ...
        if (_dropdowns['newMonitor'] == 1) CommonWidgets.buildTemplateA(null, widget.validators),
        ...
        _dropdown("Zerk Fitting Location (Left or Right, Facing Direction of Travel)",
            'zerkLocationType', ['Left', 'Right']),
        _field("Zerk Fitting Orientation", 'zerkDirection'),
        _field("Wheel Diameter", 'wheelDiameter'),
        _field("Does the Conveyor or Chain Swing, Sway, Surge, or Move Side-to-Side?",
            'conveyorSwing'),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        CommonWidgets.buildSectionTitle("Application Environment"),
        _dropdown(
            "Temperature of Area", 'appEnviroment', ['Below 30°F', 'Above 120°F', 'Normal Range']),
        _dropdown("Mounting Orientation", 'orientationType',
            ['Overhead', 'Inverted', 'Inverted/Inverted']),
        _dropdown("Are guide wheels evenly spaced?", 'guideWheelsEven', ['Yes', 'No']),
        if (_dropdowns['guideWheelsEven'] == 2)
        ...
        if (_dropdowns['mightyLubeMonitoring'] == 1)
        ...
        _field("Current Grease Type", 'currentGrease'),
        _field("Current Grease NLGI Grade", 'currentGreaseGrade'),
        _dropdown("Zerk Ftg Location (Left or Right, Facing Direction of Travel)",
            'zerkLocationType', ['Left', 'Right']),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        _field("Conveyor Length", 'conveyorLength'),
        _dropdown("Are brush applicators wanted?", 'brushApplicators', ['Yes', 'No']),
        _dropdown("Are M12 plugs required?", 'm12Plugs', ['Yes', 'No']),
      ],
    );
  }
}
